...
// constructor
  ArmPlugin::ArmPlugin()
      : ModelPlugin(), cameraNode(new gazebo::transport::Node()), collisionNode(new gazebo::transport::Node()) {
...
// Load
  void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) {
    ...
    */
    cameraSub = cameraNode->Subscribe("/gazebo/arm_world/camera/link/camera/image", &gazebo::ArmPlugin::onCameraMsg,
                                      this);
    ...
    for (unsigned int i = 0; i < contacts->contact_size(); ++i) {
      ...
      */
      if ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0)) {
        ...
        // Give reward if gripper touches object
  ...
  */
  ...
  for( uint32_t n=0; n < DOF; n++ )
  {
    ...
    */
    ...
    // Set the joint position based on whether action is even or od
    ...
    if (newReward && agent != NULL) {
      if (DEBUG) {
        printf("ArmPlugin - issuing reward %f, EOE=%s  %s\n", rewardHistory, endEpisode ? "true" : "false",
               (rewardHistory > 0.1f) ? "POS+" : (rewardHistory > 0.0f) ? "POS" : (rewardHistory < 0.0f) ? "    NEG"
                                                                                                         : "       ZERO");
      }
      ...
      if (endEpisode) {
        ...
        printf("Current Accuracy:  %0.4f (%03u of %03u)  (reward=%+0.2f %s)\n",
               float(successfulGrabs) / float(totalRuns), successfulGrabs, totalRuns, rewardHistory,
